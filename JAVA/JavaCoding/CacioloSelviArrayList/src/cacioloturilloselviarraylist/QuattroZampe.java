 ...
 */
public class QuattroZampe extends Animale implements Comparable {

    private int compare;// attributo usato per l'ordinamento dell' arrayList<>

    public QuattroZampe(String specie, Data dataNascita) {
    ...
    /**
     * equals specifico della classe QuattroZampe
     *
     * @param q, QuattroZampe
     * @return true se sono uguali, altrimenti false
     */
    public boolean equals(QuattroZampe q) {
        return (!q.isNull() && super.equals(q) && nZampe.compareTo(q.getnZampe()) == 0);
    }

    /**
     * compareTo specifico di QuattroZampe, orndina per primo il numero di zampe
     * e dopo richiama il metodo compareTo della superclasse
     *
     * @param q, Animale a QuattroZampe
     * @return -1 se questo e' piu' piccolo del parametro passato, 1 se e' piu'
     * grande e 0 se sono uguali
     */
    public Integer compareTo(QuattroZampe q) {
        if (this.nZampe.compareTo(q.getnZampe()) < 0) {
            return -1;
        }
        if (this.nZampe.compareTo(q.getnZampe()) > 0) {
            return 1;
        }
        return super.compareTo(q);
    }
    ...
    /**
     * Override del metodo, compareTo specifico della classe QuattroZampe, usato
     * per ordinare l'arrayList in base alla specie o data di nascita, sia
     * crescente che decrescente, richiamato automaticamente dalla chiamata:
