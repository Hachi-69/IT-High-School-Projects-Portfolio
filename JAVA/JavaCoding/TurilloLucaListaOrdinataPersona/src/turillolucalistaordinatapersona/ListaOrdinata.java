...
public class ListaOrdinata {
    ...
    private Integer contaPos(Nodo n) {
        Integer c = 0;
        Nodo tmp = head;
        while (tmp != null && n.getInf().getEta() > tmp.getInf().getEta()) {
            tmp = tmp.getNext();
            c++;
        }
        return c;
    ...
    private Integer cercaNodo(Nodo n) {
        ...
        while (tmp != null && !trovato && n.getInf().getEta() >= tmp.getInf().getEta()) {
            if (tmp.getInf().equals(n.getInf())) {
                trovato = true;
            }
            if (!trovato) {
                tmp = tmp.getNext();
